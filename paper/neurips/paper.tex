...
The biology motivating this design is well-established. A regulatory element such as an enhancer is an assembly of cooperatively bound TFs whose collective configuration determines transcriptional output, and the spacing and helical phase between binding events carry mechanistic information that flat per-base attribution discards. Single-cell and single-molecule assays now resolve composite binding events at near-base resolution~\citep{rauluseviciute2024jaspar}, motivating models that operate at the same level of abstraction. Treating binding events as discrete latent variables also has practical consequences: a structured event list is the input format expected by downstream tools for variant prioritization, motif-grammar discovery, and synthetic regulatory design. Recovering this representation post-hoc from attribution maps is both lossy and slow; recovering it directly is both faster and aligned with how biologists already reason about enhancer logic.
...
We minimize a weighted sum of profile reconstruction (multinomial-NLL profile shape plus log-count MSE following BPNet~\citep{avsec2021bpnet}), a Hungarian-matched set loss over (position, TF, occupancy), a slot diversity loss penalizing pairwise cosine similarity between slot attention maps, and a slot orthogonality loss on slot embeddings. Hungarian matching solves
\begin{equation}
\hat{\pi}=\arg\min_{\pi\in\Pi_K}\sum_{i=1}^{N}\mathcal{C}(\hat{e}_{\pi(i)},e_i),
\quad \mathcal{C}(\hat{e}_k, e_i)=\lambda_p (\mu_k - p_i)^2 - \lambda_t \log p_k(t_i) + \lambda_o (1 - o_k),
\end{equation}
where $\Pi_K$ is the set of injective maps from ground-truth events to slots, the position cost is squared $L_2$ in normalized coordinates, the identity cost is the negative log-probability assigned by the slot's TF head to the true class, and the occupancy cost prefers high-occupancy slots for matched events. Unmatched slots are pushed to occupancy $0$ via a small auxiliary loss, encouraging the model to use as few slots as needed. The cost weights $\lambda_p, \lambda_t, \lambda_o$ are fixed at $1$ during matching; the per-loss weights $w_*$ for the gradient pass are tuned separately and are reported below. Solving the assignment is $\mathcal{O}(K^3)$ per sequence and is dominated by sorting in our setting; for $K{=}16$ it adds negligible overhead to the forward pass.

Training uses AdamW ($\beta_1{=}0.9,\beta_2{=}0.999$, weight decay $0.01$), peak LR $1{\times}10^{-4}$ with $5\%$ linear warmup and cosine decay, batch $32$, up to $100$ epochs with patience-$20$ early stopping, gradient clipping $1.0$, and mixed precision; loss weights are profile $1$, position $1$, TF $5$, occupancy $0.5$, diversity $0.5$, orthogonality $0.1$, and site supervision $1$. At inference, BEACON does a single forward pass and returns a sorted list of slots with $o_k > \tau$ (default $\tau{=}0.5$). On a single GPU, throughput is $\sim$\,$50$ sequences/s ($\sim$\,$20$\,ms per $2$\,kb sequence) with $851$K parameters in total. By contrast, the BPNet$+$TF-MoDISco pipeline---one BPNet per TF, DeepSHAP attribution, and motif clustering---takes $\sim$\,$8.4$\,s per sequence end-to-end on the same hardware. The pipeline cost is dominated by attribution: even with batched DeepSHAP and modest $200$-sample reference distributions, per-locus attribution alone runs at $0.6$--$1.5$\,s on the same GPU, and motif clustering on the resulting attribution database adds further amortized cost. BEACON eliminates both stages because the slot heads emit the structured event list directly.
...
We also examined sensitivity to the main scaling hyperparameters. Varying the slot count $K\in\{8,16,32\}$ has a mild effect: $K{=}8$ truncates events in dense regions (mean active slots $1.6$, site $F_1$ $0.93$), $K{=}16$ is our default ($F_1$ $0.984$), and $K{=}32$ matches the default within noise while doubling the slot-attention compute. Varying the iteration count $T_{\text{iter}}\in\{1,3,5\}$ shows that $T_{\text{iter}}{=}1$ leaves slots underspecialized ($F_1$ drops to $0.91$), $T_{\text{iter}}{=}3$ is sufficient, and $T_{\text{iter}}{=}5$ does not improve metrics. Halving the slot dimension to $D_s{=}64$ saves $\sim$\,$200$K parameters but loses $\sim$\,$0.04$ in profile $r$; doubling to $D_s{=}256$ adds $\sim$\,$1.5$M parameters with no measurable benefit. These results suggest the architecture is operating in a benign region of capacity rather than at a knife-edge between under- and over-specialization, which we take as evidence that the dominant constraint is the attention-direction choice rather than raw model size.
